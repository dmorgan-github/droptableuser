...
Nset : Pproto {
	...
	*new {arg ndef, pattern;

		^super.new({

			var proxy = ndef;
			if (ndef.class == Symbol) {
				proxy = Ndef(ndef);
			};

			if (proxy.isPlaying.not) {
				proxy.play;
			};

			~args = proxy.controlNames.collect({arg ctrl; ctrl.name.asSymbol;});
			~id = proxy.nodeID;
			~type = \set;
		}, pattern);
	}
...
Nbind : Pproto {
	...
	*new {arg ndef, pattern;
		...
		^super.new({
			...
			if (Ndef(ndef.asSymbol).isPlaying.not) {
				Ndef(ndef.asSymbol).play;
			};
			~out = Ndef(ndef.asSymbol).bus;
			~group = Ndef(ndef.asSymbol).group;
		}, pattern);
	}
}
*/
	...
	*build {arg beats = 8, beatDiv = 2, phraseMaker;
		...
		var rtn = Routine({
			...
			inf.do({arg i;
				...
				if (queue.isEmpty) {
					phrase = Dphrase(isBeat, isBarStart, countToBarEnd, isDownBeat, isBackBeat, delta);
